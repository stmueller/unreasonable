...
data <- read.delim("../Model_data/IBM_SCORES.csv")
...
data$Name <- sapply(data$class,upperCase)
...
for(row in 1:nrow(scored))
{
  ...
  highest.0 <- tmp[tmp$score==max(tmp$score),]
  ##if there are more than one, pick the correct one if possible.
  highest <-highest.0[which.max(highest.0$score+highest.0$corr),]
